...
namespace MDParser.Utils
{
    public class Git
    {
        ...
        public Git(string _user, string _email, string _password, string _directory, string _repositoryHandle)
        {
            ...
            this.repository = $@"https://{_user}:{_password}@{_repositoryHandle.Replace("https://", "", StringComparison.InvariantCultureIgnoreCase)}";
            this.Email = _email;
            this.User = _user;
        ...
        {
            GitProcess p;
            await ConfigEmail(this.Email,token);
            await ConfigName(this.User, token);
        ...
        {
            GitProcess p = new GitProcess(Directory, "pull");
        ...
        {
            GitProcess p = new GitProcess(Directory, "push", $"\"{repository}\"");
        ...
        {
            GitProcess p = new GitProcess(Directory, "config", "user.name", $"\"{_name}\"");
        ...
        {
            GitProcess p = new GitProcess(Directory, "config", "user.email", $"\"{_email}\"");
